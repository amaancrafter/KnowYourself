...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
  ...
  #  result.geometry('700x550')
    ...
    Label(result, text="Result - Personality Prediction", font=titleFont, pady=10, anchor=CENTER).pack(fill=BOTH)
    
    Label(result, text = str('{} : {}'.format("Name:", aplcnt_name.get())).title(), anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Age:", age)), anchor='w').pack(fill=BOTH)
    for key in data.keys():
        if data[key] is not None:
            Label(result, text = str('{} : {}'.format(check_type(key.title()),check_type(data[key]))), anchor='w', width=60).pack(fill=BOTH)
    print(personality)
    Label(result, text = str("perdicted personality: "+personality).title(), anchor='w').pack(fill=BOTH)
...
"""
    ...
    Label(result, text = terms_mean, anchor='w', justify=LEFT).pack(fill=BOTH)
...
def perdict_person():
    ...
    lab=Label(top, text="Personality Prediction",font=titleFont, pady=10).pack()
    ...
    l1=Label(top, text="Applicant Name").place(x=70, y=130)
    l2=Label(top, text="Age").place(x=70, y=160)
    l3=Label(top, text="Gender").place(x=70, y=190)
    l4=Label(top, text="Upload Resume").place(x=70, y=220)
    l5=Label(top, text="Enjoy New Experience or thing(Openness)").place(x=70, y=250)
    l6=Label(top, text="How Offen You Feel Negativity(Neuroticism)").place(x=70, y=280)
    l7=Label(top, text="Wishing to do one's work well and thoroughly(Conscientiousness)").place(x=70, y=310)
    l8=Label(top, text="How much would you like work with your peers(Agreeableness)").place(x=70, y=340)
    l9=Label(top, text="How outgoing and social interaction you like(Extraversion)").place(x=70, y=370)
    ...
    submitBtn=Button(top, padx=2, pady=0, text="Submit", bd=0,font=(12), foreground='white', bg='red')
...
def OpenFile(b4):
    global loc;
    name = filedialog.askopenfilename(initialdir="C:/Users/Amaan/Downloads/MicroProject/KnowYourself-A Personality Prediction System/KnowYourself-A Personality Prediction System",
...
if __name__ == "__main__":
    ...
    lab=Label(root, text="Personality Prediction System", font=titleFont, pady=30).pack()
    b2=Button(root, padx=4, pady=4, width=30, text="Predict Personality", bd=1, font=homeBtnFont, command=perdict_person).place(relx=0.5, rely=0.5, anchor=CENTER)
    root.mainloop()